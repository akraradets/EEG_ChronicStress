...
\title[Chronic Stress Assessment Based on EEG Signals: An Empirical Method for EEG Feature Importance]{Chronic Stress Assessment Based on EEG Signals: An Empirical Method for EEG Feature Importance}
...
\abstract{Chronic stress is a prolonged and constant feeling of stress which could lead to depression and anxiety; therefore, a self-diagnosing tool can help notify an individual beforehand. EEG potential has been successful as a mean of prediction human emotion, but studies on EEG and chronic stress are limited. We recorded rest-state EEG and Perceived Stress Scale (PSS) score from 55 participants. Five frequency bands and asymmetry are extracted as features from the rest-state EEG. A feature selection procedure was performed using a t-test and feature importance scores from five classifiers. A RBF-SVM achieved a 10-cv score over 0.9 using eight different frequency bands, electrodes, and asymmetries namely, $\beta_{f}$, $\text{F3}_{\delta}$, $\text{F4}_{\delta}$, $\text{F3}_{\beta}$, $\text{P4}_{\delta}$, $\text{F3}_{\gamma}$, $\text{P4}_{\theta}$, and $\text{C3}_{\theta}$. This paper shows the empirical method of feature selection and confirms that EEG can be used in the chronic stress classification.}
...
\begin{figure}[h!]
    \caption{10-CV score of both SVM and LR classifier when using different rank at different number of features. A line plot show the trend of 10-CV when increasing the number of feature based on ranks. The bar plot show the 10-CV score at the number of feature, 6 for SVM and 9 for LR.}
    \label{fig:cv_svm-lr}
...
\begin{figure}[h!]
    \caption{The top six features from $\text{Rank}_{\text{Baseline}}$ and $\text{Rank}_{\text{Ensemble}}$ plot in t-SNE space. Color green and red indicate the group Non-stress and Stress respectively and shape dot and x indicates the classification result with rbf-SVM. }
...
% This study proposes a hybrid way to measure chronic stress objectively by using EEG signals as an assistant method instead of relying on the subjective view of the participant alone, which could influence the results of the questionnaires. Moreover, the result of this study might provide helpful methods by combining both psychological as well as physiological methods altogether in order to assess chronic stress. Since chronic stress can lead to mental illnesses, it is vital to examine the stress level of persons who suffer from chronic stress at its early stage by applying effective methods.

%The purpose of this study is to classify chronic stress by using resting-state EEG data. We have hypothesized that EEG can be used for classifying chronic stress. This study investigates the EEG features that contribute to the classification accuracy. This study proposes a hybrid way to measure chronic stress objectively by using EEG signals as an assistant method instead of relying on the subjective view of the participant alone, which could influence the results of the questionnaires. Moreover, the result of this study might provide helpful methods by combining both psychological as well as physiological methods altogether in order to assess chronic stress. Since chronic stress can lead to mental illnesses, it is vital to examine the stress level of persons who suffer from chronic stress at its early stage by applying effective methods.

%This study thus aims to classify chronic stress using EEG signals. This study attempts to understand the effectiveness of using EEG signals in assessing chronic stress by exploring the difference in EEG characteristics between those with chronic stress and non-stressed groups, and subsequently classifying chronic stress from the resting state using EEG signals. Finally, this study investigates the important EEG features that can be relevantly used in classifying chronic stress. 

%Since chronic stress has an impact on more than just health problems, but also desire, the performance of work as well as an attitude toward life. Thus, it might be helpful to investigate the robust and objective method using EEG signals for the evaluation of chronic stress.

%where alpha and beta can be used as signs of mental stress \cite{Puterman2011} induced by particular stimuli such as mental arithmetic tasks \cite{Vanitha-2016, Alshargie-2018} and Stroop color-word test \cite{Hou-2015, Masood-2019}.
...
%\begin{figure}[h!]
 % \centering
  %\caption{Brain Location adopted according to the 10-20 system}
...
% \begin{left}
%   \begin{tabular}{ lll }
%   1.&$\text{Rank}_{\text{Baseline}}$  & listed with no reason. \\
%   2.&$\text{Rank}_{\text{LR}}$        & listed according to the Logistic Regression Coefficient. \\
%   3.&$\text{Rank}_{\text{LDA}}$       & listed according to the Linear Discriminant Analysis Coefficient. \\
%   4.&$\text{Rank}_{\text{ADA}}$       & listed according to the feature importance score of AdaBoost. \\
%   5.&$\text{Rank}_{\text{GB}}$        & listed according to the feature importance score Gradient Boosting. \\
%   6.&$\text{Rank}_{\text{RF}}$        & listed according to the feature importance score Random Forest. \\
%   7.&$\text{Rank}_{\text{Ensemble}}$  & listed according to the ranking information of the previous five ranks. \\
%   \end{tabular}
% \end{left}
...
% From the two feature sets ($Feature_{t-test}$ and $Feature_{LR}$), Target set is obtained by following equation;

% $$ Target := \{x \mid x \text{ is in }{rank}_{1}\text{ to }{rank}_{i} \}\text{ where } 1 \leq i \leq 15 $$

% In total of 30 target set.

% T-SNE is used for visualizing the similarity of each target set along with the classification result (accuracy and cross validation score ) from both SVM and LR.
